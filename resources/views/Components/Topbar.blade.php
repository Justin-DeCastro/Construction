<div class="container-fluid bg-light d-none d-lg-block">
    <div class="row align-items-center top-bar">
        <div class="col-lg-7 col-md-12 text-center text-lg-start">
            <div class="h-100 d-inline-flex align-items-center me-6">
                <i class="fa fa-map-marker-alt text-primary me-6"></i>
                <p class="m-0">Unit 519 Vinia Residences EDSA, Brgy. Philam, Quezon City, Philippines</p>
            </div>
        </div>
        <div class="col-lg-5 col-md-12 text-end">
            <a href="https://maps.app.goo.gl/M1djPyw7viV7bGKh7" target="_blank" class="text-decoration-none">

            </a>
            <div class="h-100 d-inline-flex flex-column align-items-start me-4">
    <div class="d-inline-flex align-items-center mb-2">
        <i class="far fa-envelope-open text-primary me-2"></i>
        <a href="mailto:info@ninerconstruction.com" class="m-0 text-decoration-none text-dark">info@ninerconstruction.com</a>
    </div>
    <div class="d-inline-flex align-items-center">
        <i class="fas fa-phone-alt text-primary me-2"></i>
        <a href="tel:
0999 511 0645" class="m-0 text-decoration-none text-dark">
        0999 511 0645</a>
    </div>
</div>

            <!-- <div class="h-100 d-inline-flex align-items-center">
                    <a class="btn btn-sm-square bg-white text-primary me-1" href="https://www.facebook.com/ninerconstruction/"><i class="fab fa-facebook-f"></i></a>
                    <a class="btn btn-sm-square bg-white text-primary me-1" href=""><i class="fab fa-twitter"></i></a>
                    <a class="btn btn-sm-square bg-white text-primary me-1" href=""><i class="fab fa-linkedin-in"></i></a>
                    <a class="btn btn-sm-square bg-white text-primary me-0" href=""><i class="fab fa-instagram"></i></a>
                </div> -->
        </div>
    </div>
</div>
<!-- Topbar End -->
